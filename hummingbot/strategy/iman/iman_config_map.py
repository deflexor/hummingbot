...
def token_validate(value: str) -> Optional[str]:
    value = value.upper()
    markets = list(iman_config_map["markets"].value.split(","))
    tokens = set()
...
def derivative_market_on_validated(value: str) -> None:
    mkts = ','.split(value)
    requried_connector_trading_pairs[iman_config_map["derivative_connector"].value] = mkts

def derivative_market_prompt() -> str:
    connector = iman_config_map.get("derivative_connector").value
