...
from hummingbot.connector.exchange.binance_m import (
    binance_m_constants as CONSTANTS,
    binance_m_utils as binance_utils,
    binance_m_web_utils as web_utils,
)
from hummingbot.connector.exchange.binance_m.binance_m_api_order_book_data_source import BinanceAPIOrderBookDataSource
from hummingbot.connector.exchange.binance_m.binance_m_api_user_stream_data_source import BinanceAPIUserStreamDataSource
from hummingbot.connector.exchange.binance_m.binance_m_auth import BinanceAuth
...
class BinanceExchangeM(ExchangePyBase):
    ...
    async def _place_order(self,
        ...
        price_str = f"{price:f}"
        type_str = BinanceExchangeM.binance_order_type(order_type)
        ...
        base_currency = self.split_trading_pair(trading_pair)[0]
        if side_str == CONSTANTS.SIDE_SELL:
            borrow_result = await self._api_post(
                path_url=CONSTANTS.BORROW_PATH_URL,
                data={"asset": base_currency, "amount": amount_str},
                is_auth_required=True)
            self.logger().info(f"BORROW {base_currency} of {amount_str} {borrow_result}")
        else:
            repay_result = await self._api_post(
                path_url=CONSTANTS.REPAY_PATH_URL,
                data={"asset": base_currency, "amount": amount_str},
                is_auth_required=True)
            self.logger().info(f"REPAY {base_currency} of {amount_str} {repay_result}")
